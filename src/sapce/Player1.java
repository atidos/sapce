...
public class Player1 extends Player {
	...
	public Player1(String Name, String SpritePath, Vector2 Position, int Health){
	...
	public void Update() {
		...
		if(Inputs.LEFT && position.x>0)
		{
			origin.x -= speed * Main.deltaTime;

			Main.velocityY = -1;
		}
		if(Inputs.RIGHT && position.x<450)
		{
			origin.x += speed * Main.deltaTime;
		...
		if(Inputs.FIRE && fireCoolDown < 0)
		{
			...
			{
				Main.currentScene.Instantiate(new Bullet("Bullet", "bullet_player.png", new Vector2(position.x + 40,position.y), (float)Math.toRadians(rotation + 4),1,Enemy.class));
				Main.currentScene.Instantiate(new Bullet("Bullet", "bullet_player.png", new Vector2(position.x + 40,position.y), (float)Math.toRadians(rotation - 4),1,Enemy.class));
				Main.currentScene.Instantiate(new Bullet("Bullet", "bullet_player.png", new Vector2(position.x + 40,position.y), (float)Math.toRadians(rotation),1,Enemy.class));
			}
			else
			{
				Main.currentScene.Instantiate(new Bullet("Bullet", "bullet_player.png", new Vector2(position.x + 40,position.y), (float)Math.toRadians(rotation + Math.floor(Math.random()*(1+1+1)-1)),1,Enemy.class));
			}
